    ...
    return (
        <IonPage>
            ...
            <IonContent>
                ...
                {sneakers && (
                    <IonList>
                        {sneakers.map(({ id, name, price, owned, releaseDate}) =>
                            <SneakerItemList key={id} id={id} name={name} price={price} owned={owned} releaseDate={releaseDate} onEdit={id => history.push(`/sneaker/${id}`)} />)}
                    </IonList>
                )}
                {fetchingError && (
